...
void SetCGIStatus(const int code, FCGX_Stream *stream) {
    ...
    if(i == sizeof(HTTPreply)/sizeof(HTTPreply[0])) i = 1; // 500 Internal Server Error
    FCGX_FPrintF(stream, "Status: %d %s\n\r", code, HTTPreply[i].message);
    FCGX_SetExitStatus(code, stream);
}
